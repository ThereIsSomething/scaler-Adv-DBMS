    ...
    for (auto& name : catalog_.GetAllTableNames()) {
        ...
        ts.heap->Scan([&](const RID&, const char* data, int size) {
            ...
            if (info->schema.pk_index >= 0) {
                ...
                if (ts.stats.row_count == 1) { ts.stats.pk_min = pk; ts.stats.pk_max = pk; }
                else { ts.stats.pk_min = std::min(ts.stats.pk_min, pk); ts.stats.pk_max = std::max(ts.stats.pk_max, pk); }
            }
        });
        tables_[name] = ts;
    }
...
std::vector<Row> HeapEngine::RangeScan(const std::string& table, int low, int high) {
    auto it = tables_.find(table);
    if (it == tables_.end()) return {};
    const Schema& schema = GetSchema(table);
    if (it->second.index) {
    ...
    it->second.heap->Scan([&](const RID&, const char* data, int size) {
        ...
        if (schema.pk_index >= 0) {
            ...
            if (pk >= low && pk <= high) results.push_back(r);
        }
    });
    return results;
}
...
HeapFile* HeapEngine::GetHeapFile(const std::string& table) { return tables_.at(table).heap; }
BPlusTree* HeapEngine::GetIndex(const std::string& table) { return tables_.at(table).index; }
...
std::string HeapEngine::Execute(const Statement& stmt) {
    switch (stmt.type) {
        case StmtType::CREATE_TABLE: return ExecCreateTable(stmt.create_table);
        case StmtType::INSERT:       return ExecInsert(stmt.insert);
        case StmtType::SELECT:       return ExecSelect(stmt.select);
        case StmtType::DELETE_STMT:  return ExecDelete(stmt.delete_stmt);
        case StmtType::UPDATE:       return ExecUpdate(stmt.update);
    }
...
std::string HeapEngine::ExecSelect(const SelectStmt& stmt) {
    ...
    if (stmt.join.has_value()) {
        ...
        out_schema.columns.insert(out_schema.columns.end(), rs.columns.begin(), rs.columns.end());
    }
    ...
    if (!stmt.select_all && !stmt.columns.empty()) {
        ...
        for (auto& ref : stmt.columns) {
            ...
            if (idx >= 0) proj_schema.columns.push_back(out_schema.columns[idx]);
        }
...
std::string HeapEngine::ExecUpdate(const UpdateStmt& stmt) {
    ...
    for (auto& [rid, new_row] : to_update) {
        ...
        if (index && !(new_rid == rid)) {
            ...
            char old_buf[PAGE_SIZE]; int old_size;
...
std::string HeapEngine::FormatResults(const std::vector<Row>& rows, const Schema& schema) const {
    if (rows.empty() && schema.columns.empty()) return "(0 rows)";
